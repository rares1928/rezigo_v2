...
export default function SignIn() {
  ...
  const [rememberMe, setRememberMe] = useState(
    cookieRemember.get("rememberMe") === "true" ? true : false
  );
  ...
  const rememberChange = () => {
    ...
    cookieRemember.set("rememberMe", !rememberMe, {
      path: "/",
      maxAge: 2592000,
    });
  };
  ...
  const callLoginApi = async () => {
    ...
    await callApi(
      ...
      { email, password, tip_login: "autohton", rememberMe },
      handleLogin,
      handleError
    );
    setLoading(false);
  ...
  const redirect = () => {
    ...
    if (listaPaginiPosibile.includes(page)) {
      localStorage.removeItem("redirectPage");
      if (page === "creeaza-ti_test") {
        if (localStorage.getItem("tipTest") !== null) {
          page = page + "/" + localStorage.getItem("tipTest");
          localStorage.removeItem("tipTest");
          if (localStorage.getItem("capitoleStandard") !== null) {
            page = page + "/" + localStorage.getItem("capitoleStandard");
            localStorage.removeItem("capitoleStandard");
          }
        }
      }
      return page;
