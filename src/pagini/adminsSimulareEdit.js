...
export default function AdminsSimuareEdit() {
	...
	const deleteGrila = async (id) => {
		const url = "https://grileapiwin.azurewebsites.net/api/DeleteQuestion?code=3CivlrNptVV9mNn8G1Pz_aQWAQvIAFYaP_OEkrarraeFAzFukQliww==";
		const data = { simulareId: state, grilaId: id };
		await callApi(url, data, handleEmpty, handleError).then(getSimulareQuestions);
	};

	const updateSimulareInDB = () => {
		const updateSimulareInDBAsync = async () => {
			const url = "https://grileapiwin.azurewebsites.net/api/UpdateSimulare?code=xcfMjMllfvHPSjJZwSTvJFgioT-0s2ubfKM12PmpPKCcAzFuucpjZg==";
			const data = {
				simulareId: state,
				name: newName,
				description: newDescription,
			...
			<Paper className={classes.paper}>
				...
				<div>
					<ul>
						...
						<li>Numar grile CS: {simulareQuestions ? simulareQuestions.filter((grila) => grila.TipGrile === "CS").length : 0}</li>
						<li>Numar grile CM: {simulareQuestions ? simulareQuestions.filter((grila) => grila.TipGrile === "CM").length : 0}</li>
						<li>Numar grile total: {simulareQuestions ? simulareQuestions.length : 0}</li>
						<li>Este live (o pot vedea userii): {simulareCurenta.IsLive ? "Da" : "Nu"}</li>
					</ul>
				</div>
				<Typography className={classes.headerText} variant="h5">
					Updateaza simularea
				...
				<form onSubmit={updateSimulare} className={classes.formNewSimulare}>
					...
					<Button size="large" type="submit" className={classes.buttons} variant="contained" color="secondary" onClick={() => updateSimulareInDB()}>
						{" "}
						Updateaza simularea
					</Button>
				</form>
			...
			<Grid container spacing={3}>
				<Grid item xs={6}>
					<Paper className={classes.paper}>
						...
						<AdminsDisplayGrile simID={simulareCurenta.ID} getSimulareQuestions={getSimulareQuestions} />
					</Paper>
				</Grid>
				<Grid item xs={6}>
					<Paper className={classes.paper}>
