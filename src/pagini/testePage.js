...
import testStandard from "../poze/testStandard.svg";
...
export default function TestePage() {
    ...
    const [cookiesAccord, setCookiesAccord] = useState(localStorage.getItem("cookiesAccord") === "true" ? true : false);
    ...
    const handleCategorii = (e) => {
        ...
        const listaTipTest = ["test_nou", "grile_pe_pagini", "simulare", "teste_incepute", "reparcurge_greseli", "examene_rezidentiat", "test_standard"];
        let cardToSelect = "";
        ...
        setReadyCat(true);
        if (cardToSelect === "Test Standard" && window.location.pathname.split("/")[3]) {
    ...
    const cautaGrilePerPage = async () => {
        ...
        const url = "https://grileapiwin.azurewebsites.net/api/getcategoriiperpage?code=0XsuIZIk34HpyUs/xDAF6DFHk/xIS9ovT7aVCCn7UMkMb0SMM3nyZg==";
        let carte = "";
        if (cartePagini === "Kumar și Clark Medicină Clinică") {
            carte = "Kumar";
        } else if (cartePagini === "Chirurgie generală și specialități chirurgicale") {
            carte = "Chirurgie";
        } else {
            carte = "Sinopsis";
        }
    ...
    const creeazaSimulare = async () => {
        ...
        vectorNumarGrile = vectorNumarGrile.sort(
            (a, b) => a["subcategorie"].reduce((acc, val) => acc + val["numar"], 0) - b["subcategorie"].reduce((acc, val) => acc + val["numar"], 0)
        );
        const mediaPerCapitol = Math.floor(200 / listaSelectiiSimulare.filter((capitol) => capitol).length);
        let d = 200 - mediaPerCapitol * listaSelectiiSimulare.filter((capitol) => capitol).length;
        ...
        for (let i = 0; i < vectorNumarGrile.length; i++) {
            grileDeAdaugat = 0;
            grilePerCapitol = vectorNumarGrile[i]["subcategorie"].reduce((acc, val) => acc + val["numar"], 0);
            ...
            mediaPerSubCapitol = Math.floor(grileDeAdaugat / vectorNumarGrile[i]["subcategorie"].length);
            dSubcat = grileDeAdaugat - mediaPerSubCapitol * vectorNumarGrile[i]["subcategorie"].length;
            for (let j = 0; j < vectorNumarGrile[i]["subcategorie"].length; j++) {
                grileDeAdaugatSubcat = 0;
                grilePerSubCapitol = vectorNumarGrile[i]["subcategorie"].sort((a, b) => a["numar"] - b["numar"])[j]["numar"];

                if (i === vectorNumarGrile.length - 1 && j === vectorNumarGrile[i]["subcategorie"].length - 1) {
                ...
                lista_categorii.push({
                    ...
                    nume_subcategorie: vectorNumarGrile[i]["subcategorie"].sort((a, b) => a["numar"] - b["numar"])[j]["nume_subcategorie"],
        ...
        const url = "https://grileapiwin.azurewebsites.net/api/CreateTestWin?code=UWWieYZbXJombLLaR12BaLqCxfdBbHEz84QWnVaE/ZCVyCm2Fi9nvg==";
        await callApi(url, { lista_categorii, aleator: true }, handleTestIdNou, handleError);
        setGoLoading(false);
    };
        ...
        for (let i = 0; i < listaselectii.length; i++) {
            for (let j = 0; j < listaselectiisubcat[i].length; j++) {
                if (listaselectiisubcat[i][j] > 0 && intrebariRamase > 0) {
                    ...
                    intrebariRamase = intrebariRamase - Math.min(intrebariRamase, listaselectiisubcat[i][j]);
                }
            }
        }
    ...
    const creeazaTestPerPage = async () => {
            ...
            for (let j = 0; j < listaselectiisubcatPerPage[i].length; j++) {
                if (listaselectiisubcatPerPage[i][j] > 0) {
                    lista_categorii.push({
                        ...
                        nume_subcategorie: listaCategoriiPerPage[i]["subCategory"][j]["Name"],
        ...
        const url = "https://grileapiwin.azurewebsites.net/api/createtestperpage?code=W6TOWV3TlKt4Z4MDlmvET21aB4IJrDsYaNS8fDLkpq/E7zjaEJWt7g==";
    ...
    const displayTestStandard = () => {
        return (
            <>
                <div className={classes.bookDiv}>
                    <Typography variant="h6" className={classes.instructionsText}>
                        2. Selectează cărțile și capitolele din care dorești un test standard:
                    </Typography>
                    <Grid className={classes.cardGrid} container justifyContent="center" spacing={4}>
                        <Grid item className={classes.bookLevel}>
                            <TestsBookCard
            ...
            <>
                <div className={classes.bookDiv}>
                    ...
                    <Grid className={classes.cardGrid} container justifyContent="center" spacing={4}>
                        ...
                        <Grid item className={classes.bookLevel}>
                            <TestsBookCard
                                isSelected={cartePagini === "Chirurgie generală și specialități chirurgicale"}
                                setCardSelected={setCartePagini}
                                imagine={lawrence}
                                title="Chirurgie generală și specialități chirurgicale"
                                grilePePagini={true}
                                ...
                                <div>
                                    ...
                                    <div className={classes.bookSubcatDiv}>
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => {
                                                setFirstPage(event.target.value === "" ? null : Number(event.target.value));
                                            }}
                                        />
                                        <Typography variant="subtitle2" className={classes.instructionsText}>
                                            Până la pagina:
                                        ...
                                        <Input
                                            ...
                                            onChange={(event) => {
                                                setSecondPage(event.target.value === "" ? 0 : Number(event.target.value));
                                            }}
                                        />
                                        <Button
                                            className={classes.marginCls}
                                            color="secondary"
                                            variant="contained"
                                            disabled={firstPage > secondPage || loadingPerPage === true}
                                            onClick={cautaGrilePerPage}
                                        >
                                            {loadingPerPage ? <CircularProgress /> : <div>Caută grile</div>}
                                        </Button>
                                        {/* <Button
                      className={classes.marginCls}
                      color="primary"
                      ...
                      Altă căutare
                            ...
                            {readyPerPage &&
                                (listaCategoriiPerPage.length === 0 ? (
                                    <Typography variant="h6" className={classes.instructionsText}>
                                        Nu am găsit grile în intervalul de pagini specificat de tine.
                                    </Typography>
                                ) : (
                ...
                <div className={classes.bookDiv}>
                    {tipCont === "Standard" ? (
                        <>
                            ...
                            <Typography variant="h6">Pentru a selecta un capitol apasă pe pătratul din dreptul acestuia.</Typography>
                        </>
                    ) : (
                        <Typography variant="h6" className={classes.instructionsText}>
        ...
        return (
            <div>
                ...
                <div>
                    {
                        <DataTable
                            rows={listatTesteNeterm.sort((a, b) => a.Done - b.Done).filter((test) => test["ReparcurgeGreseli"] === false)}
        ...
        return (
            <div>
                ...
                <div>
                    {
                        <DataTable
                            rows={listatTesteNeterm.filter((test) => test["ReparcurgeGreseli"] === true)}
                            onDelete={(id) => deleteTest(id)}
                            onClick={(id) => handleTestId(id)}
                        />
                    }
    ...
    const onClickCategorieMarePerPage = (i) => {
        ...
        if (
            ...
            listaCategoriiPerPage[i].subCategory.reduce((acc, subcat) => acc + subcat.Count, 0)
        ) {
            for (let index = 0; index < listaselectiisubcatPerPage[i].length; index++) {
            ...
            for (let index = 0; index < listaselectiisubcatPerPage[i].length; index++) {
                lista_temporara[index] = listaCategoriiPerPage[i].subCategory[index].Count;
            }
            lista_temporara_mare[i] = lista_temporara;
            setListaselectiisubcatPerPage(lista_temporara_mare);
        }
    ...
    const numarGrileStandard = () => {
        return listaSelectiiStandard.reduce(
            (acc, currentValue, index) =>
                acc + (currentValue ? listaCategorii[index]["subCategory"].reduce((acc, currentSubcat) => acc + currentSubcat["GrileStandardNumber"], 0) : 0),
            0
        );
    };
    ...
    return (
        <div className={classes.root}>
            ...
            <CookiesAccord cookiesAccord={cookiesAccord} setCookiesAccord={setCookiesAccord} />
            ...
            >
              Buton popup
                        ...
                        <Grid className={classes.cardGrid} justifyContent="center" container spacing={4} id="testCard_div">
                            <Grid item>
                                <TestsCard
                ...
                {isCardSelected === "Test nou" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Test nou"} timeout={growTimeout}>
                        <div>{displayTestNou()}</div>
                    </Grow>
                )}
                {isCardSelected === "Grile pe pagini" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Grile pe pagini"} timeout={growTimeout}>
                        <div>{displayGrilePePagini()}</div>
                    </Grow>
                )}
                {isCardSelected === "Simulare" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Simulare"} timeout={growTimeout}>
                        <Typography>
                ...
                {isCardSelected === "Teste începute" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Teste începute"} timeout={growTimeout}>
                        <div>{displayTestNeterminat()}</div>
                    </Grow>
                )}
                {isCardSelected === "Reparcurge greșeli" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Reparcurge greșeli"} timeout={growTimeout}>
                        <div> {displayReparcurgeGreseli()} </div>
                    </Grow>
                )}
                {isCardSelected === "Examene rezidențiat" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Examene rezidențiat"} timeout={growTimeout}>
                        <div> {displayExameneRezidentiat()} </div>
                    </Grow>
                )}
                {isCardSelected === "Test Standard" && (
                    <Grow id="bookCard_div" in={isCardSelected === "Test Standard"} timeout={growTimeout}>
                        <Typography>
                        ...
                        <Slide
                            in={
                                ...
                                (isCardSelected === "Test nou" || isCardSelected === "Examene rezidențiat"
                                    ? sumaElemArr(listaselectiisubcat) > 0
                                    : sumaCategoriiArray(listaSelectiiSimulare) > 0 || sumaCategoriiArray(listaSelectiiStandard) > 0)
                                        ...
                                        <Grid className={classes.footerItem} item>
                                            ...
                                            isCardSelected === "Examene rezidențiat" ? (
                                                <Button
                                                    ...
                                                    color="secondary"
                                                    onClick={() => {
                                                        isCardSelected === "Grile pe pagini" ? setAleatorPerPage(!aleatorPerPage) : setAleator(!aleator);
                                                    }}
                                                >
                                                    {isCardSelected === "Grile pe pagini" ? (
                                                        <Typography variant="subtitle2">
                                                            {aleatorPerPage ? <>Grile randomizate</> : <>Grile ordonate</>}
                                                        </Typography>
                                                    ) : (
                                                        <>
                                                            {aleator ? (
                                                                <Typography variant="subtitle2">Grile randomizate</Typography>
                                                            ) : (
                                                                <Typography variant="subtitle2">Grile ordonate</Typography>
                                                            )}
                                                        </>
                                                    )}
                                                </Button>
                                            ) : isCardSelected === "Test Standard" ? (
                                                <Typography variant="subtitle2">Ordinea grilelor: ordonate</Typography>
                                            ) : (
                                                <Typography variant="subtitle2">Ordinea grilelor: aleatoare</Typography>
                                            )}
                                        </Grid>
                                        <Grid className={classes.footerItem} item>
                                            <Button
                                                ...
                                                disabled={
                                                    goLoading ||
                                                    (isCardSelected === "Simulare" ? produsScalarListe(listaSelectiiSimulare) <= 200 : questionRemaining === 0)
                                                }
                                                onClick={
                                                    isCardSelected === "Test nou" || isCardSelected === "Examene rezidențiat"
                                                        ? () => creeazaTest()
                                            ...
                                            >
                                                {goLoading ? <CircularProgress color="primary" size={25} /> : <Typography>Ready Set GO!</Typography>}
                                            </Button>
                                            {(isCardSelected === "Test nou" || isCardSelected === "Examene rezidențiat") &&
                                            ...
                                            {isCardSelected === "Simulare" && produsScalarListe(listaSelectiiSimulare) <= 200 && (
                                                <>
                                                    <Typography variant="subtitle2" className={classes.errorTooManyQ}>
