...
export default function AdminsDisplayGrile({ simID, getSimulareQuestions }) {
  ...
  useEffect(() => {
    ...
    const url =
      "https://grileapiwin.azurewebsites.net/api/GetCategoriiAdmin?code=S5MkCQa6aai1rR47PEL1AB3QwiaG2Fe522pJLONtsmhHGmPj6re45w==";
    const data = {};
  ...
  const cautaGrileReports = async () => {
    ...
    setItems({});
    const url =
      "https://grileapiwin.azurewebsites.net/api/reportsgrilerreturnadmin?code=MUOhM3OBWspRxt2XbeEfsiWSazEJMI4DbH2Z5iVmRfKt15jNrLFxyg==";
  ...
  const addQuestionToSim = async (grilaID) => {
    const url =
      "https://grileapiwin.azurewebsites.net/api/AddQuestionToSimulare?code=XLRsha5UvXRHSWjduHZKUbJusqu5GMkSi1H5Z1D1Le8gAzFuhdVcjQ==";
    const data = { grilaId: grilaID, simulareId: simID }; //aici mai trebuie verificat cu DBx
    await callApi(url, data, handleSimQuestion, handleError).then(
        ...
        {!ready ? (
          ...
          <>
            <Typography className={classes.selectInstructions}>
              Alege, in aceasta ordine cartea, capitolul, subcapitolul. Ordinea
              repriznta modul in care vor fi afisate grilele.
            </Typography>
            <Paper className={classes.paper}>
              <div className={classes.paper}>
                ...
                >
                  ...
                  >
                    ...
                    <MenuItem value={"Rezidențiat 2021"}>
                      Rezidențiat 2021
                    </MenuItem>
                  </Select>
                </FormControl>
                ...
                >
                  ...
                  >
                    ...
                    {capitol === -1
                      ? null
                      : listaCat[capitol]["subCategory"].map(
                          (subcategorie, index) => (
                            <MenuItem
                              key={
                                carte +
              ...
              <Typography className={classes.typography}>
                Vreau ca enuntul grilei sa contina acest pasaj: (lasa
                necompletat daca doresti sa vezi toate grilele)
              </Typography>
                  ...
                  <div className={classes.grileListDiv}>
                    {items["lista"]
                      ...
                      .map((grilaPrimita, index) => (
                        <div
                          className={classes.grilaDiv}
                          key={
                            "capitolul_" + capitol + "_grila:" + String(index)
                          }
                        >
