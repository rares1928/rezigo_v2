            ...
            {rows.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell className={classes.tableBody} align="center">
                  {row.CreatedAt.split("T")[0]} ora:{" "}
                  {row.CreatedAt.split("T")[1].replace(":00", "")}
                </TableCell>
                <TableCell className={classes.tableBody} align="center">
                  {row.NumAnswered + row.NumUnanswered}
                </TableCell>
                <TableCell className={classes.tableBody} align="center">
                  {row.NumAnswered}
                </TableCell>
                ...
                <TableCell className={classes.tableBody} align="center">
                  {!row.Done ? (
                    ...
                    <>
                      {" "}
                      {row.ScorRezi}/{row.ScorReziPosibil} (
                      {Number(
                        (row.ScorRezi / row.ScorReziPosibil) * 950
                      ).toFixed(0)}
                      /950){" "}
                    </>
                  )}
